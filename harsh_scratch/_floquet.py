...
class FloquetAnalysis(Serializable):
    ...
    def run_one_floquet(
        ...
        f_modes_0 = fbasis.mode(0.0, data=True)
        f_energies = fbasis.e_quasi
        sampling_time = self.options.floquet_sampling_time_fraction * T % T
        f_modes_t = (
            fbasis.mode(sampling_time, data=True) if sampling_time != 0.0 else f_modes_0
        )
        return f_modes_t.T, f_energies

    def identify_floquet_modes(
        self,
        f_modes_energies: tuple[np.ndarray, np.ndarray],
        ...
        """
        f_modes, _ = f_modes_energies
        (omega_d_0, amp_0) = params_0

        ideal_displaced_state_array = self.displaced_state(
            np.array([omega_d_0]), np.array([[amp_0]]), self.state_indices, coefficients
        )[:, 0, 0, :]

        # TODO: refactor using overlap_with_bare_states method?
        # Issue: nested parallelization. overlap_with_bare_states is parallelized over all cpus.
        # However, this fn sits inside _run_floquet_and_calculate, which is itself parallelized.
        ovlps_and_modes = np.zeros(
            (len(self.state_indices), 1 + self.hilbert_dim), dtype=complex
        )
        overlaps = np.einsum("ik,jk->ij", np.conj(ideal_displaced_state_array), f_modes)

        # take the argmax along k
        f_idxs = np.argmax(np.abs(overlaps), axis=1)
        for array_idx, _ in enumerate(self.state_indices):
            ...
            ovlps_and_modes[array_idx, 1:] = (
                np.sign(bare_state_overlap) * f_modes[:, f_idx]
            )
        return ovlps_and_modes

    def _step_in_amp(
        self, f_modes_energies: tuple[np.ndarray, np.ndarray], prev_f_modes: np.ndarray
    ...
    def run(self, filepath: str | None = None) -> dict:
        ...
        for amp_range_idx in range(num_fit_ranges):
            ...
            new_coefficients = displaced_state.displaced_states_fit(
                ovlp_with_bare_states, floquet_modes_for_range
            )
    ...
    def _floquet_main_for_amp_range(
        ...
        # TODO: edge case of None
        num_amps = amp_idxs[1] - amp_idxs[0]
        num_omega_d = omega_d_idxs[1] - omega_d_idxs[0]

        def _run_floquet_and_calculate(
            omega_d_idx: int,
        ) -> tuple[np.ndarray, np.ndarray, np.ndarray, np.ndarray]:
            # Arrays to store results
            avg_excitation = np.zeros((num_amps, self.hilbert_dim))
            quasienergies = np.zeros_like(avg_excitation)
            ovlps = np.zeros((num_amps, len(self.state_indices)), dtype=complex)
            modes = np.zeros(
                (num_amps, len(self.state_indices), self.hilbert_dim), dtype=complex
            )

            prev_f_modes_for_omega_d = prev_f_modes[omega_d_idx]

            params_0 = (omega_d, amps[0, omega_d_idx])  # TODO
        ...
        all_avg_excitation = np.array(all_avg_excitation).reshape(
            (len(self.model.omega_d_values), len(amps), self.hilbert_dim)
        )
